...
function catchTheMenu(embed, day) {
	...
	embed = embed
		.setTitle('Menu')
		.setDescription('Menu du ' + days[day] + ' ' + Menu[0][day - 1] + ' au soir')
...
function menusoir(message, prefix) {
	const today = new Date();
	// Get the option
	let option = message.content.replace('menusoir', '').replace(prefix, '').toLowerCase();
	while (option.includes(' ')) {
		option = option.replace(' ', '');
	}
	// Get the menu of the week from the .csv file
	parseFile('menusoir.csv', { delimiter: ';' })
		.on('data', row => {
			...
			if (option == '') {
				...
				if (!isNaN(option)) {
					...
					if (occurencies != -1) {
						...
						message.channel.send('Je suis désolé, mais le menu n\'est pas disponible pour le ' + option + ' au soir');
						return;
					}

				} else {
					message.channel.send('La syntaxe est ' + prefix + 'menusoir {paramètre}, tapez ' + prefix + 'help pour obtenir de l\'aide.');
					return;
				}
			}
